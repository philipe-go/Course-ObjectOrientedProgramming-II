using PacmanGame.Classes.Pathfinding;
using System;
using System.Collections.Generic;
using System.Drawing;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace PacmanGame.Classes.Entities
{
    class Booster : AbstractEntity
    {
        public Booster(int row, int column) : base (15, row, column) {}

        public override void Draw(Graphics g)
        {
            int x = this.Location.X + (Map.cellSize/4);
            int y = this.Location.Y + (Map.cellSize/4);
            int side = Map.cellSize/2;

            Rectangle bounds = new Rectangle(x, y, side, side);
            Brush myBrush = new SolidBrush(this.Color);
            g.FillEllipse(myBrush, bounds);

            myBrush.Dispose();
        }
    }
}
